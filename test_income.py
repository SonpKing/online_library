...
class library_config():
    ...
    def get_DueFine(self):
        return self.DueFine
    def get_DamageFine(self):
        return self.DamageFine
    def get_LostFine(self):
        return self.LostFine
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if data is not None:
            ...
            sql1 = "insert into reader(certificateNo, name, password, registerTime) values (\'%s\',\'%s\',\'%s\',\'%s\')" % (certificateNo, name, password, date)
            cur.execute(sql1)
            #type 3：定金
            cur_date = datetime.datetime.now()
            curDate = datetime.date(cur_date.year,cur_date.month,cur_date.day)
            sql_deposit = "insert into incomeRecord(recordTime,type,value) values (\'%s\',3,%d)"%(curDate,library.get_deposit())
...
def rebook():
    ...
    if request.method =='POST':
        ...
        fine = 0
        if (curDate-dueDate).days>0:
            fine = (curDate-dueDate).days*library.get_DueFine()
        ...
        #图书损坏
            ...
            fine_damage = library.get_DamageFine()+price
            cur_fine = mysql.get_db().cursor()
            ...
            fine_lost = library.get_LostFine()+price
            cur_fine = mysql.get_db().cursor()
...
def get_fine_by_bookID():
    ...
    if request.method =='GET':
        ...
        fine_lost =0
        if (curDate-dueDate).days>0:
            fine_overdue = (curDate-dueDate).days*library.get_DueFine()
